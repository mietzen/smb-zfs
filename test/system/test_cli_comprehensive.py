...
def test_duplicate_user_creation(comprehensive_setup):
    ...
    with pytest.raises(subprocess.CalledProcessError):
        run_smb_zfs_command("create user comp_user1 --password 'DuplicatePass!' --json")

def test_duplicate_group_creation(comprehensive_setup):
    """Test creating duplicate groups returns appropriate error."""
    with pytest.raises(subprocess.CalledProcessError):
        run_smb_zfs_command("create group comp_group1 --description 'Duplicate group' --json")

def test_duplicate_share_creation(comprehensive_setup):
    """Test creating duplicate shares returns appropriate error."""
    with pytest.raises(subprocess.CalledProcessError):
        run_smb_zfs_command("create share comp_share1 --dataset shares/comp_share1_dup --json")
...
def test_nonexistent_group_operations(comprehensive_setup):
    ...
    with pytest.raises(subprocess.CalledProcessError):
        run_smb_zfs_command("modify group nonexistent_group --add-users comp_user1 --json")
...
def test_nonexistent_share_operations(comprehensive_setup):
    ...
    with pytest.raises(subprocess.CalledProcessError):
        run_smb_zfs_command("modify share nonexistent_share --comment 'New comment' --json")
...
def test_user_with_multiple_groups(comprehensive_setup):
    ...
    run_smb_zfs_command("modify group comp_group1 --add-users comp_user3 --json")
    run_smb_zfs_command("modify group comp_group2 --add-users comp_user3 --json")
...
def test_share_with_complex_permissions(comprehensive_setup):
    ...
    run_smb_zfs_command("create share complex_share --dataset shares/complex_share --valid-users comp_user1,@comp_group1,comp_user3 --owner comp_user1 --group comp_group1 --perms 770 --json")
...
def test_quota_operations(comprehensive_setup):
    ...
    assert get_zfs_property('primary_testpool/homes/comp_user1', 'quota') == '15G'
    
    # Change quota
    run_smb_zfs_command("modify home comp_user1 --quota 25G --json")
    assert get_zfs_property('primary_testpool/homes/comp_user1', 'quota') == '25G'
    
    # Remove quota
    run_smb_zfs_command("modify home comp_user1 --quota none --json")
    assert get_zfs_property('primary_testpool/homes/comp_user1', 'quota') == 'none'
...
def test_modify_share_all_options(comprehensive_setup):
    ...
    run_smb_zfs_command("create share modify_all --dataset shares/modify_all --json")
    
    # Modify all possible options
    run_smb_zfs_command("modify share modify_all --comment 'Fully modified share' --valid-users comp_user1,comp_user2 --owner comp_user1 --group comp_group1 --perms 755 --quota 30G --readonly --no-browse --json")
    ...
    assert get_zfs_property('primary_testpool/shares/modify_all', 'quota') == '30G'

def test_modify_setup_all_options(comprehensive_setup):
    """Test modifying all setup options."""
    # Test all setup modification options
    run_smb_zfs_command("modify setup --server-name FULLTEST --workgroup FULLGROUP --macos --default-home-quota 40G --json")
...
def test_special_characters_in_names(comprehensive_setup):
    ...
    run_smb_zfs_command("create user test_user_123 --password 'TestPass!' --json")
    run_smb_zfs_command("create group test_group_456 --description 'Test group with numbers' --json")
...
def test_long_descriptions_and_comments(comprehensive_setup):
    ...
    run_smb_zfs_command(f"create group long_desc_group --description '{long_description}' --json")
    run_smb_zfs_command(f"create share long_comment_share --dataset shares/long_comment_share --comment '{long_description}' --json")
...
def test_state_consistency_after_operations(comprehensive_setup):
    ...
    run_smb_zfs_command("create user state_test --password 'StateTest!' --json")
    run_smb_zfs_command("modify group comp_group1 --add-users state_test --json")
    run_smb_zfs_command("create share state_share --dataset shares/state_share --valid-users state_test --json")
    ...
    assert len(final_state['samba']['shares']) == len(initial_state['samba']['shares']) + 1

def test_cleanup_operations(comprehensive_setup):
    """Test that cleanup operations work correctly."""
    # Create temporary resources
    run_smb_zfs_command("create user cleanup_user --password 'CleanupPass!' --json")
    run_smb_zfs_command("create group cleanup_group --json")
    run_smb_zfs_command("create share cleanup_share --dataset shares/cleanup_share --json")
    ...
    run_smb_zfs_command("delete share cleanup_share --delete-data --yes --json")
