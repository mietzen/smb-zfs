...
def test_multiple_operations_sequence(initial_state):
    ...
    assert get_zfs_property('primary_testpool/shares/workflow_share', 'quota') == '15G'
    assert get_zfs_property('primary_testpool/homes/workflow_user', 'quota') == '5G'

# --- Test modify operations with boolean flags ---
def test_modify_share_boolean_flags(initial_state):
    """Test share modification with boolean flag variations."""
    run_smb_zfs_command("create share bool_share --dataset shares/bool_share --json")
...
# --- Test cleanup and deletion edge cases ---
def test_delete_user_with_group_membership(initial_state):
    """Test deleting user who is member of groups."""
    run_smb_zfs_command("create user member_user --password 'MemberPass!' --json")
...
def test_delete_group_with_members(initial_state):
    ...
    run_smb_zfs_command("create user group_member --password 'GroupMemberPass!' --json")
    run_smb_zfs_command("create group member_group --users group_member --json")
...
def test_delete_share_with_dependencies(initial_state):
    ...
    run_smb_zfs_command("create user share_user --password 'ShareUserPass!' --json")
    run_smb_zfs_command("create share dependent_share --dataset shares/dependent_share --valid-users share_user --json")
...
# --- Test comprehensive remove scenarios ---
def test_remove_with_complex_setup():
    ...
    run_smb_zfs_command("setup --primary-pool primary_testpool --secondary-pools secondary_testpool tertiary_testpool --server-name COMPLEXTEST --workgroup COMPLEXGROUP --macos --default-home-quota 25G")

    # Add complex data
    run_smb_zfs_command("create user complex1 --password 'Complex1!' --shell --json")
    run_smb_zfs_command("create user complex2 --password 'Complex2!' --no-home --json")
    run_smb_zfs_command("create group complex_group --users complex1 --json")
    run_smb_zfs_command("create share complex_share1 --dataset shares/complex_share1 --pool primary_testpool --json")
    run_smb_zfs_command("create share complex_share2 --dataset shares/complex_share2 --pool secondary_testpool --valid-users @complex_group --json")
    ...
    assert get_zfs_property('primary_testpool/homes/complex1', 'type') == 'filesystem'
    assert get_zfs_property('primary_testpool/shares/complex_share1', 'type') == 'filesystem'
...
def test_password_security_handling(initial_state):
    ...
    run_smb_zfs_command("create user security_user --password 'SecretPassword123!' --json")

    # Verify user was created successfully
    assert get_system_user_details('security_user') is not None

# --- Test dataset naming and structure ---
def test_dataset_structure_consistency(initial_state):
    ...
    run_smb_zfs_command("create user struct_user --password 'StructPass!' --json")
    run_smb_zfs_command("create share struct_share --dataset shares/struct_share --json")

    # Verify consistent naming structure
    assert get_zfs_property('primary_testpool/homes/struct_user', 'type') == 'filesystem'
    assert get_zfs_property('primary_testpool/shares/struct_share', 'type') == 'filesystem'

    # Verify mountpoints are correct
    assert get_zfs_property('primary_testpool/homes/struct_user', 'mountpoint') == '/home/struct_user'
...
def test_smb_conf_consistency(initial_state):
    ...
    from conftest import read_smb_conf

    # Create share
    run_smb_zfs_command("create share conf_share --dataset shares/conf_share --comment 'Config test share' --readonly --json")
    ...
    run_smb_zfs_command("modify share conf_share --comment 'Modified config test' --json")
...
def test_system_user_integration(initial_state):
    ...
    run_smb_zfs_command("create user sys_user --password 'SysPass!' --shell --json")
...
def test_comprehensive_workflow(initial_state):
    ...
    for dept in departments:
        run_smb_zfs_command(f"create group {dept} --description '{dept.capitalize()} department' --json")
    ...
    for username, dept in users:
        run_smb_zfs_command(f"create user {username} --password '{username.capitalize()}Pass!' --shell --json")
        run_smb_zfs_command(f"modify group {dept} --add-users {username} --json")

    # 3. Create departmental shares
    for dept in departments:
        run_smb_zfs_command(f"create share {dept}_share --dataset shares/{dept} --valid-users @{dept} --quota 100G --comment '{dept.capitalize()} department share' --json")

    # 4. Create a common share
    run_smb_zfs_command("create share common --dataset shares/common --valid-users alice,bob,carol,dave --comment 'Common shared area' --json")
    ...
    for username, _ in users:
        ...
        assert get_zfs_property(f'primary_testpool/homes/{username}', 'quota') == '10G'
    ...
    for dept in departments:
        ...
        assert get_zfs_property(f'primary_testpool/shares/{dept}', 'quota') == '100G'
