...
def test_wizard_setup_with_options() -> None:
    """Test the setup wizard with non-default options."""
    # Clean up any existing setup first
    run_smb_zfs_command("remove --delete-users --delete-data --yes")

    cmd = "wizard setup"
    inputs = [
        "primary_testpool",             # Primary pool
        "secondary_testpool",           # Secondary pools
        "MACSERVER",                    # Server name
        "MACGROUP",                     # Workgroup
        "y",                            # macOS optimized?
        "20G",                          # Default home quota
        "y"                             # Proceed?
    ]
    result = run_smb_zfs_command(cmd, user_inputs=inputs)
    check_wizard_output(result, "Setup completed successfully.")
...
def test_wizard_modify_setup_settings(initial_state) -> None:
    """Test modifying server settings via the wizard."""
    cmd = "wizard modify setup"
    inputs = [
        "primary_testpool",             # Keep primary pool
        "secondary_testpool",           # Remove tertiary pool
        "NEWSERVER",                    # New server name
        "NEWGROUP",                     # New workgroup
        "n",                            # Disable macOS optimization
        "none"                          # Remove default home quota
    ]
    result = run_smb_zfs_command(cmd, user_inputs=inputs)
    check_wizard_output(result, "Global setup modified successfully.")
    ...
    assert 'server string = NEWSERVER' in smb_conf
    assert MACOS_SETTINGS not in smb_conf


def test_wizard_remove_complete(initial_state) -> None:
    """Test the full remove wizard, deleting users and data."""
    # Create an item to ensure it gets deleted
    run_smb_zfs_command(
        "create user sztest_w_rm_user --password 'SecretPassword!'")
    assert get_system_user_exists('sztest_w_rm_user')
    assert get_zfs_dataset_exists('primary_testpool/homes/sztest_w_rm_user')
    ...
    # Verify state
    assert not get_system_user_exists('sztest_w_rm_user')
    assert not get_zfs_dataset_exists('primary_testpool/homes')
    # Check that a core command fails, indicating the state file is gone
    error_result = run_smb_zfs_command("list users")
    assert "Error: System not set up. Run 'setup' first." in error_result
