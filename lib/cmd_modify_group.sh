# Modify group business logic function
modify_group_business_logic() {
    local groupname="$1"
    local users_to_add="$2"
    local users_to_remove="$3"
...
cmd_modify_group() {
    ...
    read -r -p "Add users to group (comma-separated): " users_to_add
    read -r -p "Remove users from group (comma-separated): " users_to_remove

    # Call business logic function
    modify_group_business_logic "$groupname" "$users_to_add" "$users_to_remove"
}