# Modify setup business logic function
modify_setup_business_logic() {
    local new_server_name="$1"
    local new_workgroup="$2"
    local new_macos="$3"
...
cmd_modify_setup() {
    ...
    read -r -p "Enter new server name [current: $current_server_name]: " new_server_name
    if [[ -z "$new_server_name" ]]; then
        new_server_name="$current_server_name"
    fi

    read -r -p "Enter new workgroup [current: $current_workgroup]: " new_workgroup
    if [[ -z "$new_workgroup" ]]; then
        new_workgroup="$current_workgroup"
    fi

    read -r -p "Enable macOS optimization? (y/n) [current: $current_macos]: " macos_input
    ...
    modify_setup_business_logic "$new_server_name" "$new_workgroup" "$new_macos"
}
