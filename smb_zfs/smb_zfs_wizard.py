...
def confirm_destructive_action(message):
    ...
    print(f"To proceed, you must type the following phrase exactly: {CONFIRM_PHRASE}", file=sys.stderr)
    response = input("> ")
...
def wizard_create_share(manager, args=None):
    ...
    try:
        ...
        dataset_path = prompt("Enter the ZFS dataset path within the pool (e.g., data/media)")
        if not dataset_path:
        ...
        result = manager.create_share(share_name, dataset_path, owner, group, perms, comment, valid_users, read_only, browseable)
        print(f"\nSuccess: {result}")
...
def wizard_delete_user(manager, args=None):
    ...
    try:
        ...
        if delete_data:
            if not confirm_destructive_action(f"This will PERMANENTLY delete user '{username}' AND their home directory."):
                return
...
def wizard_delete_share(manager, args=None):
    ...
    try:
        ...
        if delete_data:
            if not confirm_destructive_action(f"This will PERMANENTLY delete the ZFS dataset for share '{share_name}'."):
                return
...
def main():
    ...
    p_delete_share = delete_sub.add_parser('share', help='Start the delete share wizard.')
    p_delete_share.set_defaults(func=wizard_delete_share)
    p_delete_group = delete_sub.add_parser('group', help='Start the delete group wizard.')
    p_delete_group.set_defaults(func=wizard_delete_group)

    p_uninstall = subparsers.add_parser('uninstall', help='Start the uninstallation wizard.')
