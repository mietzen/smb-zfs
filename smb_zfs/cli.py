...
def confirm_destructive_action(prompt, yes_flag):
    ...
    print(f"To proceed, type the following phrase exactly: {CONFIRM_PHRASE}", file=sys.stderr)
    response = input("> ")
...
def cmd_install(manager, args):
    ...
    if args.dry_run:
        ...
        print(f"  - Install packages: samba, samba-common-bin, avahi-daemon")
        print(f"  - Create ZFS dataset: {args.pool}/homes")
        print(f"  - Create system group: smb_users")
        ...
        print(f"  - Enable and start smbd, nmbd, avahi-daemon services")
...
def cmd_create_user(manager, args):
    ...
    if args.dry_run:
        ...
        print(f"  - Create ZFS home dataset: {manager._state.get('zfs_pool')}/homes/{args.user}")
        print(f"  - Set permissions on home directory")
        print(f"  - Add Samba user: {args.user}")
        print(f"  - Add user to group 'smb_users'")
        if groups:
...
def cmd_create_share(manager, args):
    ...
    if args.dry_run:
        print("--- Dry Run ---")
        print("Would perform the following actions:")
        print(f"  - Create ZFS dataset: {manager._state.get('zfs_pool')}/{args.dataset}")
        print(f"  - Set ownership to {args.owner}:{args.group} and permissions to {args.perms}")
        print(f"  - Add share '{args.share}' to {manager._config.SMB_CONF}")
        print(f"  - Reload Samba configuration")
        print(f"  - Update state file")
        return
...
def cmd_delete_user(manager, args):
    ...
    if args.dry_run:
        ...
        if args.delete_data:
            print(f"  - DESTROY ZFS dataset: {manager._state.get_item('users', args.user)['home_dataset']}")
        print(f"  - Update state file")
        return

    if args.delete_data:
        if not confirm_destructive_action(f"This will permanently delete user '{args.user}' AND their home directory.", args.yes):
...
def cmd_delete_share(manager, args):
    ...
    if args.dry_run:
        ...
        if args.delete_data:
            print(f"  - DESTROY ZFS dataset: {manager._state.get_item('shares', args.share)['dataset']}")
        print(f"  - Reload Samba configuration")
        print(f"  - Update state file")
        return

    if args.delete_data:
        if not confirm_destructive_action(f"This will permanently delete the ZFS dataset for share '{args.share}'.", args.yes):
            print("Operation cancelled.", file=sys.stderr)
            return
...
def main():
    ...
    # --- Install ---
    p_install = subparsers.add_parser('install', help='Initial setup of Samba, ZFS, and Avahi.')
    p_install.add_argument('--pool', required=True, help='The name of the ZFS pool to use.')
    p_install.add_argument('--server-name', help='The server\'s NetBIOS name (default: hostname).')
    p_install.add_argument('--workgroup', help='The workgroup name (default: WORKGROUP).')
    p_install.add_argument('--macos', action='store_true', help='Enable macOS compatibility optimizations.')
    p_install.add_argument('--dry-run', action='store_true', help="Don't change anything just summarize the changes")
    p_install.set_defaults(func=cmd_install)

    # --- Create ---
    p_create = subparsers.add_parser('create', help='Create a new user, share, or group.')
    ...
    p_create_user.add_argument('--password', help="Set the user's password. If omitted, will prompt securely.")
    p_create_user.add_argument('--shell', action='store_true', help='Grant the user a standard shell (/bin/bash).')
    p_create_user.add_argument('--groups', help='A comma-separated list of groups to add the user to.')
    p_create_user.add_argument('--dry-run', action='store_true', help="Don't change anything just summarize the changes")
    ...
    p_create_share.add_argument('--dataset', required=True, help="The path for the ZFS dataset within the pool (e.g., 'data/projects').")
    p_create_share.add_argument('--comment', default="", help='A description for the share.')
    p_create_share.add_argument('--owner', default='root', help='The user who will own the files (default: root).')
    p_create_share.add_argument('--group', default='smb_users', help='The group that will own the files (default: smb_users).')
    p_create_share.add_argument('--perms', default='775', help="File system permissions for the share's root (default: 775).")
    p_create_share.add_argument('--valid-users', help="Comma-separated list of users/groups allowed to connect. Use '@' for groups.")
    p_create_share.add_argument('--readonly', action='store_true', help='Make the share read-only (default: no).')
    p_create_share.add_argument('--no-browse', action='store_true', help='Hide the share from network browsing (default: browseable).')
    p_create_share.add_argument('--dry-run', action='store_true', help="Don't change anything just summarize the changes")
    ...
    p_create_group.add_argument('--description', default="", help='A description for the group.')
    p_create_group.add_argument('--users', help='A comma-separated list of initial members.')
    p_create_group.add_argument('--dry-run', action='store_true', help="Don't change anything just summarize the changes")
    ...
    p_delete_user.add_argument('--delete-data', action='store_true', help='Permanently delete the associated ZFS dataset.')
    p_delete_user.add_argument('--yes', action='store_true', help="Assume 'yes' to destructive confirmation prompts.")
    p_delete_user.add_argument('--dry-run', action='store_true', help="Don't change anything just summarize the changes")
    ...
    p_delete_share.add_argument('--delete-data', action='store_true', help='Permanently delete the associated ZFS dataset.')
    p_delete_share.add_argument('--yes', action='store_true', help="Assume 'yes' to destructive confirmation prompts.")
    p_delete_share.add_argument('--dry-run', action='store_true', help="Don't change anything just summarize the changes")
    ...
    p_delete_group.add_argument('--dry-run', action='store_true', help="Don't change anything just summarize the changes")
    ...
    p_list.add_argument('type', choices=['users', 'shares', 'groups'], help='The type of item to list.')
    ...
    # --- Uninstall ---
    p_uninstall = subparsers.add_parser('uninstall', help='Remove all configurations, data, and packages.')
    p_uninstall.add_argument('--delete-data', action='store_true', help='Permanently delete all associated ZFS datasets.')
    p_uninstall.add_argument('--delete-users', action='store_true', help='Permanently delete all users and groups created by this tool.')
    p_uninstall.add_argument('--yes', action='store_true', help="Assume 'yes' to destructive confirmation prompts.")
